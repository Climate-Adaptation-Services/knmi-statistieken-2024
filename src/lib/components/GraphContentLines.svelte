...
<g class='graph-content-lines'>
  <line stroke={'black'} y2={yScale(datalist_laag[0].p5)} y1={yScale(datalist_hoog[0].p95)} x1={xScale('Current climate')} x2={xScale('Current climate')} stroke-dasharray="8 8" stroke-width='2.8'/>
  {#each [['laag', datalist_laag], ['hoog', datalist_hoog]] as datalist}
    ...
    <g class='median-line scenario-{datalist[0]}' stroke='{(datalist[0] === 'laag') ? '#17A3D3' : 'red'}'>
      <line x1={xScale('Current climate')} x2={xScale('2050')} y1={yScale(datalist[1][0].median)} y2={yScale(datalist[1][1].median)}/>
